import {buildKitchen} from './scene-option3.mjs';
...
await new Promise((resolve,reject)=>new GLTFExporter().parse(root,async data=>{await fs.writeFile(new URL('./kitchen-option3.glb',import.meta.url),Buffer.from(data));resolve();},reject,{binary:true}));
await fs.writeFile(new URL('./dimensions-option3.json',import.meta.url),JSON.stringify(root.userData,null,2));

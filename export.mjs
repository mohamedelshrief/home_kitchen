import {buildKitchen} from './scene.mjs';
...
await new Promise((resolve,reject)=>new GLTFExporter().parse(root,async data=>{await fs.writeFile(new URL('./alex-kitchen.glb',import.meta.url),Buffer.from(data));resolve();},reject,{binary:true}));
await fs.writeFile(new URL('./dimensions.json',import.meta.url),JSON.stringify(root.userData,null,2));
